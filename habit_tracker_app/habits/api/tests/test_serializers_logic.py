...
class HabitViewsAPITests(APITestCase):
    def setUp(self):
        self.username1 = 'admin_api_serializers'
        self.password1 = 'api_serializers1234'
        ...
        self.habit_weekly = create_habit(self.user1, 'api habit weekly', 'api habit purp', 'weekly', 2)

    def test_api_show_habit_logs_divided_into_blocks_for_weekly_habit(self):
        '''
            Проверка, что список логов для недельной привычки (для api detail), отображается в виде списка, состоящих из блоков.
            В качестве примера возьмём количество логов = 13
        '''
        habit = Habit.objects.get(datetype='weekly')
        create_habit_log(habit, 'New log 1', 'complited', 1)
        logs_count = 13
        for _ in range(2, logs_count+1):
            create_habit_log(habit, f'New log {_} день назад', 'incomplited', _)

        self.client.login(username=self.username1, password=self.password1)
        response = self.client.get(reverse("api:habit-detail", args=(habit.id, )))

        self.assertEqual(type(response.data["habit_logs"]), list)
        self.assertEqual(len(response.data["habit_logs"][0]), 7)
        self.assertEqual(len(response.data["habit_logs"][1]), logs_count-7)

    def test_api_creation_habitlogs_with_status_forgot_to_mark(self):
        '''
            Проверка api создания логов привычки, которые не были созданы пользователем в промежутке хотя бы в два дня между последним логом и текущей датой.
        ...
        self.client.login(username=self.username1, password=self.password1)
        response = self.client.post(reverse('api:api_create_habit_log', args=(habit.id, )), data)
    ...
    def test_api_increase_streak_for_habit_datetype_daily_with_no_logs_added_before(self):
        '''Проверка увеличения поля streak при создании лога у ежедневной привычки, у которой до этого не было логов. При этом статус созданного лога - complited'''
        habit = Habit.objects.get(datetype='daily')
        data = generate_habit_log_data('Log com', HABIT_LOG_STATUS_COMPLITED)

        self.client.login(username=self.username1, password=self.password1)
        self.client.post(reverse('api:api_create_habit_log', args=(habit.id, )), data)
    ...
    def test_api_increase_streak_for_habit_datetype_daily_with_logs_added_before(self):
        '''Проверка увеличения поля streak у ежедневной при создании лога у ежедневной привычки, у которой последний лог со статусом complited. При этом статус созданного лога - complited'''
        habit = Habit.objects.get(datetype='daily')
        habit.streak += 1
    ...
    def test_api_zeroing_out_streak_for_habit_datetype_daily_with_logs_added_before(self):
        ...
        data = generate_habit_log_data('Log com', HABIT_LOG_STATUS_INCOMPLITED)
        self.client.login(username=self.username1, password=self.password1)

        self.client.post(reverse('api:api_create_habit_log', args=(habit.id, )), data)
        self.assertEqual(Habit.objects.get(datetype='daily').streak, 0) # streak стал из 1 -> 0

    def test_api_increase_streak_for_habit_datetype_weekly_when_created_log_is_complited(self):
        '''
        ...
        for _ in range(2, 7):
            create_habit_log(habit, f'New log {_} день назад', 'incomplited', _)
        data = generate_habit_log_data('Log for today', HABIT_LOG_STATUS_COMPLITED)
        self.client.login(username=self.username1, password=self.password1)
        self.client.post(reverse('api:api_create_habit_log', args=(habit.id, )), data)
        
        self.assertEqual(Habit.objects.get(datetype='weekly').streak, 1)

    def test_api_increase_streak_for_habit_datetype_weekly_while_created_log_is_incompleted_and_habitlogs_with_status_complited_is_over_frequency(self):
        '''
        ...
        data = generate_habit_log_data('Log for today', HABIT_LOG_STATUS_INCOMPLITED)
    ...
    def test_api_no_increase_streak_for_habit_datetype_weekly_when_created_log_is_complited_but_habitlogs_not_multiple_of_7(self):
        '''
        ...
        for _ in range(2, 7):
            create_habit_log(habit, f'New log {_+1} день назад', 'incomplited', _)
        data = generate_habit_log_data('Log', HABIT_LOG_STATUS_INCOMPLITED)
    ...
    def test_api_no_increase_streak_for_habit_datetype_weekly_when_created_log_is_incomplited(self):
        '''
            Проверка НЕ увеличения поля streak у еженедельной привычки, если кол-во логов со статусом complited < периодичности привычки (frequency).
            При этом созданный лог имеет статус - incomplited и кол-во созданных логов кратно 7
        '''
        ...
        self.client.login(username=self.username1, password=self.password1)
        self.client.post(reverse('habits:set_habit_status_for_today', args=(habit.id, )), data)

        self.assertEqual(Habit.objects.get(datetype='weekly').streak, 0)

    def test_api_zeroing_out_streak_for_habit_datetype_weekly(self):
        '''
        ...
        data = generate_habit_log_data('Last log', HABIT_LOG_STATUS_INCOMPLITED)
