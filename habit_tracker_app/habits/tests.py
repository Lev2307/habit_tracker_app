...
class HabitViewTest(TestCase):
    ...
    def test_habit_create_page_is_login_required(self):
        '''Проверка, что только авторизованный пользователь может зайти на страницу создания привычки (GET, POST методы соответственно)'''
    ...
    def test_create_habit_weekly_wrong_data(self):
        ...
        self.assertEqual(Habit.objects.filter(title='Change mindset wrong 1').exists(), False)

        self.assertEqual(response_wrong_data2.status_code, 200)
        self.assertEqual(Habit.objects.filter(title='Change mindset wrong').exists(), False)
